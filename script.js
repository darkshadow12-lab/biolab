...
const introDuckFamily = document.getElementById("introDuckFamily");
...
introScreen.addEventListener("click", function () {
  ...
  if (introDuckFamily) {
    introDuckFamily.style.animation = "introFamilyExit 1.05s ease-in forwards";
  }
...
const duckLayer = document.querySelector(".background-duck-layer");
...
function getSafeSpots() {
  return [
    ...
    { x: 50, y: window.innerHeight - 170 },
    { x: window.innerWidth - 160, y: window.innerHeight - 180 },
    { x: window.innerWidth * 0.2, y: window.innerHeight - 220 },
    { x: window.innerWidth * 0.82, y: 210 }
  ];
}
...
function clampInside(position) {
  ...
  return {
    x: Math.max(padding, Math.min(window.innerWidth - 120, position.x)),
    y: Math.max(padding, Math.min(window.innerHeight - 120, position.y))
  };
}

function setDuckPosition(position, allowOutside = false) {
  duckPosition = allowOutside ? position : clampInside(position);
...
function getDuckCenter() {
  const rect = pixelDuck.getBoundingClientRect();
...
function getMouseSpeed(event) {
  ...
  const distance = getDistance(currentMousePosition, lastMousePosition);
  const timeDiff = Math.max(1, now - lastMouseTime);
...
function getFarthestSpot(mouse) {
  ...
  spots.forEach(function (spot) {
    const distance = getDistance(spot, mouse);
...
function getNaturalMove() {
  const moveXOptions = [-30, -22, 22, 30];
  const moveYOptions = [-10, -6, 6, 10];
...
function startPanicMovement() {
  clearPanicTimeouts();
  clearTimeout(duckAmbientTimeout);
  ...
  for (let i = 0; i < steps; i++) {
    const timeoutId = setTimeout(function () {
      setDuckPosition(getNaturalMove());
    }, i * delay);
...
function hideDuck() {
  ...
  const exitPosition = {
    x: exitLeft ? -220 : window.innerWidth + 220,
    y: duckPosition.y
  };

  setDuckPosition(exitPosition, true);

  clearTimeout(duckReturnTimeout);

  duckReturnTimeout = setTimeout(function () {
    const spots = getSafeSpots();
    ...
    const outsidePosition = {
      x: spawnLeft ? -180 : window.innerWidth + 180,
      y: returnSpot.y
    };

    duckScareCount = 0;

    setDuckPosition(outsidePosition, true);
...
function scareDuck(mousePosition) {
  ...
  setTimeout(function () {
    if (duckScareCount >= DUCK_ESCAPE_LIMIT) {
      hideDuck();
      return;
    }

    const farthestSpot = getFarthestSpot(mousePosition);
...
function makeDuckCurious() {
  if (duckState !== "sleeping" && duckState !== "idle") {
    return;
  }
...
function scheduleAmbientGrooming() {
  ...
  duckAmbientTimeout = setTimeout(function () {
    if (duckState !== "sleeping") {
      scheduleAmbientGrooming();
      return;
    }

    setDuckState("grooming");

    setTimeout(function () {
      if (duckState === "grooming") {
        setDuckState("sleeping");
        scheduleAmbientGrooming();
      }
    }, 1400);
...
function maybeSpawnRareDuck() {
  ...
  const rareDuck = document.createElement("div");
  rareDuck.className = "rare-duck";

  const rareDuckPixel = document.createElement("div");
  rareDuckPixel.className = "rare-duck-pixel";

  rareDuck.appendChild(rareDuckPixel);
  duckLayer.appendChild(rareDuck);

  const y = window.innerHeight - 120 - Math.random() * 120;
  const fromLeft = Math.random() > 0.5;

  const startX = fromLeft ? -80 : window.innerWidth + 80;
  const endX = fromLeft ? 70 : window.innerWidth - 120;

  rareDuck.style.transform =
    `translate(${startX}px, ${y}px) scale(0.82)`;

  setTimeout(function () {
    rareDuck.classList.add("visible");
    rareDuck.style.transform =
      `translate(${endX}px, ${y}px) scale(0.82)`;
  }, 100);

  setTimeout(function () {
    rareDuck.classList.remove("visible");
  }, 3200);
...
function scheduleRareDuck() {
  clearTimeout(rareDuckTimeout);

  const delay = 35000 + Math.random() * 50000;

  rareDuckTimeout = setTimeout(function () {
    maybeSpawnRareDuck();
    scheduleRareDuck();
  }, delay);
}
...
function handleMouseMove(event) {
  ...
  const mouseSpeed = getMouseSpeed(event);
  const duckCenter = getDuckCenter();
  const distance = getDistance(mousePosition, duckCenter);

  if (distance < DUCK_DANGER_DISTANCE) {
    scareDuck(mousePosition);
    return;
  }

  if (
    distance < DUCK_CURIOUS_DISTANCE &&
    mouseSpeed < 0.55 &&
    duckState === "sleeping"
  ) {
    makeDuckCurious();
...
window.addEventListener("resize", refreshDuckPosition);
...
const takeOffMarker = document.getElementById("takeOffMarker");
...
clearHistoryBtn.addEventListener("click", function () {
  const confirmed =
    confirm("Tüm kayıtları silmek istediğine emin misin?");
